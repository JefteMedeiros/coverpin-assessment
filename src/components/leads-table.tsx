...
export function LeadsTable() {
  const { data: leads, error, isLoading, sortByScore, sortDescending } = useData();
            ...
            <TableRow>
              ...
              <TableHead className="flex items-center gap-2">
                ...
                <button
                  type="button"
                  onClick={sortByScore}
                  title={`Sort ${sortDescending ? "ascending" : "descending"}`}
                >
                  <ArrowDownUp
                    className={`w-4 h-4 transition-transform ${sortDescending ? "rotate-180" : ""}`}
                  />
                </button>
              </TableHead>
