		...
		return (
			<div className="p-4 bg-zinc-800 space-y-4 rounded-lg">
				<p className="text-center py-8 text-muted-foreground">
					No opportunities found. Convert some leads to see them here.
				</p>
			</div>
		);
	}
					...
					<TableBody>
						{isLoading ? (
							...
							opportunities.map((opportunity) => (
								<TableRow key={opportunity.id} className="hover:bg-muted/20 transition-colors">
									<TableCell title={opportunity.name}>{nameFormatter(opportunity.name)}</TableCell>
									<TableCell className="hidden md:table-cell">{opportunity.accountName}</TableCell>
									<TableCell>
										<Badge className={getStageColor[opportunity.stage]} variant="default">
											{opportunity.stage}
										</Badge>
									</TableCell>
